...
class Metasequoia : BaseModel {
	...
	public override EAxisOrder AxisOrder { get { return EAxisOrder.XYZ; } }
	public override EAxisDir AxisDir { get { return EAxisDir.XpYpZp; } }
	...
	void writeMaterial(StreamWriter fs) {
		...
		foreach (var m in mMaterialList) {
			var val = m.Value;
			var col = val.Diffuse ^ 1;
			fs.Write("\t\"{0}\"", val.Name.Replace("\"", ""));
			fs.Write(" shader(3)");
			fs.Write(" col({0} {1} {2} {3})", col.x, col.y, col.z, val.Alpha);
			fs.Write(" dif({0})", val.Diffuse.Norm);
			fs.Write(" amb({0})", val.Ambient.Norm);
			fs.Write(" emi(0)");
			fs.Write(" spe({0})", val.Specular.Norm);
